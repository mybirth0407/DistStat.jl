...
function _get_grouplasso_args(λ::T, idx::Vector{Ti}) where {T <: Real, Ti <: Integer}
    @assert idx[1] == 1
    # 순차적으로 1부터 ... N까지 그룹이 반드시 존재한다고 가정
...
function prox!(y::AbstractArray{T}, f::GroupNormL2{T,A}, x::AbstractArray{T}, indices::Vector{Int}, γ::T=one(T); unpen::Int=length(x) - f.p) where {T <: Real, A <: AbstractArray}
    unpen=10
    # f.p가 2000
    println(indices)
    println(size(f.grpmat)) # 2000, 100
    println(size(f.tmp_g))
    # println(typeof(f.grpmat))
    # println(size(@view(y[1:end-unpen])))
    # println(size(transpose(f.grpmat[indices[1]:indices[2], :])))
    # println(size(f.grpmat[indices[1]:indices[2], :]))
    y[1:end-unpen] .= @view(x[1:end-unpen]) .^ 2
    # 100, (2000 * 100)^T, 2000
    mul!(f.tmp_g, transpose(f.grpmat[indices[1]:indices[2], :]), @view(y[1:end-unpen]))
    f.tmp_g .= sqrt.(f.tmp_g) # groupwise norms
    f.tmp_g .= (γ .* f.max_norms) ./ (max.(γ .* f.max_norms, f.tmp_g)) # multiplication factors
    gather!(f.tmp_p, f.tmp_g, f.gidx)
    # println(size(@view(x[1:end-unpen])))
    y[1:end-unpen] .= (1 .- f.tmp_p[indices[1]:indices[2]]) .* @view(x[1:end-unpen])
...
function value(f::GroupNormL2{T,A}, x::AbstractVector{T}, splits::Vector{Int}, rank::Int; unpen::Int=length(x) - f.p) where {T <: Real, A <: AbstractArray}
    # f.tmp_p .= @view(x[1:end-unpen]) .^ 2
    # println("size of f.tmp_p: ", size(f.tmp_p))
    # println("f.tmp_p: ", f.tmp_p)
    idxs = [0; cumsum(splits)]
    f.tmp_p[1+idxs[rank+1]:idxs[rank+2]] .= @view(x.localarray[1:end-unpen]) .^ 2
    mul!(f.tmp_g, transpose(f.grpmat), f.tmp_p)
    f.tmp_g .= sqrt.(f.tmp_g)
